...
def getContours(npImage, overlay_img, realHeight, realWidth, unit, confidence, angle_th=30):
    ...
    for c in cnts:
        ...
        if slope > 0:
            ...
            cv2.putText(orig, f"angle={angle:.1f}", (max(top_p[0] - 100, 0), top_p[1] + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
        else:
            angle = np.arccos((bot_p[0] - top_p[0]) / D_ad) * 180 / math.pi
            cv2.putText(orig, f"angle={angle:.1f}", (top_p[0], top_p[1] + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
        ...
        if angle < angle_th:
            cat += 'H'
            cv2.putText(orig, f"L={length:.1f}{unit}", (int(tltrX), int(tltrY) + 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
            cv2.putText(orig, f"W={width:.1f}{unit}", (int(tltrX), int(tltrY) + 55),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
        else:
            cat += 'V'
            cv2.putText(orig, f"L={length:.1f}{unit}", (int(tltrX), int(tltrY)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)
            cv2.putText(orig, f"W={width:.1f}{unit}", (int(tltrX), int(tltrY) + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 1)

        cat += 'L' if slope > 0 else 'R'

        cv2.putText(orig, f"cat={cat} Crack percentage={confidence.item():.2f}",
                    (x1, max(0, y1 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (36, 255, 12), 1)

        return orig, length, width, angle, cat, confidence.item()

    # If no valid contour found
    return overlay_img, 0, 0, 0, '', confidence.item()
